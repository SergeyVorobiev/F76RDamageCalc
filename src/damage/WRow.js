    ...
    return (
        <Col>
            <InputGroup className="mb-1 mt-1 flex-nowrap">
                <WarningPopover message={message} header={header} element={<InputGroup.Text color='red' style={{ width: '2.58rem', backgroundColor: '#e8f4ff', cursor: 'pointer' }}>{icon}</InputGroup.Text>} />
...
function column3(wSpec, onChange, icon, name, id, step, max, min, message) {
    ...
    return (
        <Col key={id}>
            <InputGroup className="mb-1 mt-1 flex-nowrap">
                <WarningPopover message={message} header={name} element={<InputGroup.Text color='red' style={{ width: '2.58rem', backgroundColor: '#e8f4ff', cursor: 'pointer' }}>{icon}</InputGroup.Text>} />
...
const WRow = memo(function WRow({wSpec, setWSpec}) {
    ...
    return (
        <Row>
            ...
            {column3(wSpec, onChange, '⌛', "Trigger Delay", "triggerDelay", 0.1, 3, 0.1, "Delay between hits for manual guns.")}
            {column3(wSpec, onChange, '⌛', "Auto Delay", "autoDelay", 0.1, 3, 0.1, "Delay between hits for automatic guns.")}
            {column3(wSpec, onChange, '⌛', "Attack Delay", "attackDelay", 0.1, 3, 0.1, "Delay between hits for melee weapons.")}
            {column(wSpec, onChange, ammo("1.1rem"), "Ammo Capacity", "ammoCapacity", 1, 9999, 0)}
            {column(wSpec, onChange, "🛡️", "Anti Armor", "aa", 1, 100, 0)}
            {column3(wSpec, onChange, "💪", "Strength Boost", "strengthBoost", 1, 20, 0, "Boost in percents for each strength point (Melee).")}
            {column3(wSpec, onChange, "☠️", "Crit", "crit", 1, 999, 0, "Crit bonus, default is +100% meaning 2x of base damage.")}
            {column3(wSpec, onChange, "🐍", "Sneak", "sneak", 1, 999, 0, "Sneak bonus, default is +100% meaning 2x of base damage.")}
            {column(wSpec, onChange, "🌪️", "Bash", "bash", 1, 200, 0)}
            {column3(wSpec, onChange, "⚔️", "Attack Damage", "attackDamage", 0.1, 5, 0.1, "Attack damage multiplier, modifies base non-time damage.")}
            {column3(wSpec, onChange, "🗡️", "Outgoing Damage", "outgoingDamageMult", 0.1, 3, 0.5, "Outgoing damage multiplier, multiples damage after passing creature's resistance.")}
            {column3(wSpec, onChange, "💣", "Explosive", "exp", 1, 100, 0, "Explosive part, percentage of total damage.")}
            {creatureViews}
            ...
            {column3(wSpec, onChange, "🎯", "Accuracy", "accuracy", 1, 100, 5, "General accuracy of a weapon: accuracy * damage accuracy * damage chance.")}
