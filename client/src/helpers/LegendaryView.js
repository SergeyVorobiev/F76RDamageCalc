...
export default function LegendaryView({template}) {
    ...
    if (isLeg1 || isLeg2 || isLeg3 || isLeg4 || isLeg5) {
        const leg1 = (isLeg1) ? keyValueBadge("bg-yellow m-1", '20rem', "⭐",  template.legendary1.name) : (<></>);
        const leg2 = (isLeg2) ? keyValueBadge("bg-yellow m-1", '20rem', "⭐⭐",  template.legendary2.name) : (<></>);
        const leg3 = (isLeg3) ? keyValueBadge("bg-yellow m-1", '20rem', "⭐⭐⭐",  template.legendary3.name) : (<></>);
        const leg4 = (isLeg4) ? keyValueBadge("bg-yellow m-1", '20rem', "⭐⭐⭐⭐",  template.legendary4.name) : (<></>);
        const leg5 = (isLeg5) ? keyValueBadge("bg-yellow m-1", '20rem', "⭐⭐⭐⭐⭐",  template.legendary5.name) : (<></>);
        return (
            <>
